...
# Script is written to create users with  root privilages only 
...
# Additionally all the outputs are not get to the terminal

# Checking if the user is Root user i.e. it should have user ID equal to 0

if [[ "${UID}"  -ne 0 ]]
then
        echo "$(id -un) is not root user and hence cannot execute this..!!" &>> /dev/null
...
then
        echo "No username is provided. Please provide username of the user to be created." &>> /dev/null
        exit 1
fi
...
adduser -c "${comment}" -m "${username}" &>> /dev/null                     # Comment is used that is equal to all the argumets passed than 1st i.e. 1st chopped

if [[ "${?}" -ne 0 ]]                                                      # Checking if account cretion is successful by checking exit status of recent 
then                                                                       # most command executed 
        echo "Something went wrong in account creation..!!" &>> /dev/null             
        exit 1
fi
...
echo "${password}" | passwd --stdin "${username}" &>> /dev/null            # Providing password to password command

if [[ "${?}" -ne 0 ]]                                                      # Checking if password cretion is successful by checking exit status of recent most command
then
        echo "Something went wrong in password creation..!!" &>> /dev/null            
        exit 1
fi

passwd -e "${username}" # &>> /dev/null                                    # Force change password on 1st login (this is not sent to null as I want to show that password expires after first login) 
